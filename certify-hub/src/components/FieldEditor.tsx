    ...
    <div className="mt-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs text-gray-500">Adjust text details</span>
          <button
            type="button"
            onClick={() => setShowDetails(!showDetails)}
            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 ${
              showDetails ? 'bg-indigo-600' : 'bg-gray-200'
            }`}
          >
            <span
              ...
              <>
                ...
                <div className="flex items-center gap-2 flex-wrap mb-2">
                  <input
                    ...
                    className="w-32 px-2 py-1 rounded border border-gray-300 text-sm bg-gray-100"
                    disabled={true}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    value={field.label}
                    onChange={e => onFieldLabelChange(field.id, e.target.value)}
                    className="w-full px-2 py-1 rounded border border-gray-300 text-sm bg-gray-100"
                    disabled={true}
                    placeholder="Field label"
                  />
                </div>
      ...
      {/* Add field section - only show in detailed view */}
      {showDetails && (
